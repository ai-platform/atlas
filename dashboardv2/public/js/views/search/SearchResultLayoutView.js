    ...
    /** @lends SearchResultLayoutView */
    {
        ...
        this.listenTo(
          ...
          function (model, response) {
            ...
            if (errorText) {
              ...
              this.$(".searchTable > .well").html(
                "<center>" + _.escape(errorText) + "</center>"
              );
            }
          },
          this
          ...
          function (state) {
            if (Utils.getUrlState.isSearchTab()) {
              ...
              if (this.value && this.value.type) {
                excludeDefaultColumn = _.difference(
              ...
              this.columnOrder = this.getColumnOrder(
                this.REntityTableLayoutView.$el.find(".colSort th.renderable")
              );
              this.triggerUrl();
      ...
      onRender: function () {
        ...
        this.commonTableOptions = {
          ...
          includeColumnManager:
            Utils.getUrlState.isSearchTab() &&
            this.value &&
            this.value.searchType === "basic" &&
            !this.profileDBView
              ? true
              : false,
          ...
          columnOpts: {
            ...
            visibilityControlOpts: {
              buttonTemplate: _.template(
                "<button class='btn btn-action btn-sm pull-right'>Columns&nbsp<i class='fa fa-caret-down'></i></button>"
              ),
            },
        ...
        if (!this.initialView) {
          ...
          if (
            ...
            this.searchTableColumns[this.value.type] === null
          ) {
            this.ui.columnEmptyInfo.show();
          } else {
            this.ui.columnEmptyInfo.hide();
          ...
          this.fetchCollection(
            ...
            _.extend(
              { fromUrl: true },
              this.value && this.value.pageOffset ? { next: true } : null
            )
          );
          ...
          if (this.value && this.value.pageLimit) {
            this.ui.showPage
              .val(this.limit)
              .trigger("change", { skipViewChange: true });
          }
          this.$(".entityLink").show();
      ...
      updateColumnList: function (updatedList) {
        if (updatedList) {
          ...
          if (this.value && this.value.type && this.searchTableColumns) {
            this.searchTableColumns[this.value.type] = listOfColumns.length
              ? listOfColumns
              : null;
          }
        } else if (
      ...
      fetchCollection: function (value, options) {
        var that = this,
          ...
          entityFilters = null;
        if (isSearchTab) {
          tagFilters = CommonViewFunction.attributeFilter.generateAPIObj(
            this.value.tagFilters
          );
          entityFilters = CommonViewFunction.attributeFilter.generateAPIObj(
            this.value.entityFilters
          );
        }

        if (isPostMethod && isSearchTab) {
          var excludeDefaultColumn =
              this.value.type && this.searchTableColumns
                ? _.difference(
                    this.searchTableColumns[this.value.type],
                    this.defaultColumns
                  )
        ...
        if (
          ...
          Globals.searchApiCallRef.readyState === 1
        ) {
        ...
        var apiObj = {
          ...
          success: function (dataOrCollection, response) {
            ...
            if (
              ...
              Utils.notifyInfo({
                ...
                content:
                  ...
                  "<b>" +
                  Utils.getNumberSuffix({ number: pageNumber, sup: true }) +
                  "</b> page",
              });
              return;
            }
            if (isPostMethod) {
              ...
              that.searchCollection.fullCollection.reset(
                dataOrCollection.entities,
                { silent: true }
              );
            }
             ...
             ' <div class="searchTypeLabel"><b>'+that.searchType+'</b></div>'+
              ...
              '<div class="entityLink searchTypeSubLabel">'+
               ...
               searchString +=
                ...
                CommonViewFunction.generateQueryOfFilter(that.value) +
                "</span>";
              if (
                Globals.entityCreate &&
                Globals.entityTypeConfList &&
                Utils.getUrlState.isSearchTab()
              ) {
                searchString +=
                  "<p class='searchResultsSubText'>If you do not find the entity in search result below then you can" +
        ...
        if (value) {
          ...
          if (this.profileDBView && value.typeName && value.guid) {
            ...
            profileParam["relation"] =
              value.typeName === "hive_db"
                ? "__hive_table.db"
          ...
          if (isPostMethod) {
            ...
            apiObj["data"] = _.extend(
              ...
              filterObj,
              _.pick(
                this.searchCollection.queryParams,
                "query",
      ...
      tableRender: function (options) {
              ...
              _.each(this.models, function (model, index) {
                model.set(
                  ...
                  (savedColumnOrder == null
                    ? index
                    : parseInt(savedColumnOrder[model.get("label")])) + 1,
                  { silent: true }
                );
              });
              return this;
        ...
        var columns = new columnCollection(
          that.searchCollection.dynamicTable
            ? that.getDaynamicColumns(that.searchCollection.toJSON())
            : that.getFixedDslColumn()
        );
        columns.setPositions().sort();
        if (
          this.searchType == "Advanced Search" &&
          columns.length &&
          that.searchCollection.length
        ) {
        ...
        var table = new TableLayout(
          _.extend({}, that.commonTableOptions, {
            columns: columns,
          })
        );
        ...
        if (that.isTableDropDisable !== true) {
          var tableDropFunction = function (from, to, el) {
            ...
            that.columnOrder = that.getColumnOrder(
              el.querySelectorAll("th.renderable")
            );
            that.triggerUrl();
            that.tableRender({ order: that.columnOrder, table: TableLayout });
            that.checkTableFetch();
          };
          that.REntityTableLayoutView.$el
            .find(".colSort thead tr th:not(.select-all-header-cell)")
            .addClass("dragHandler");
      ...
      renderTableLayoutView: function (col) {
        ...
        require(["utils/TableLayout"], function (TableLayout) {
          ...
          if (that.value.uiParameters) {
            var savedColumnOrder = _.object(
              that.value.uiParameters.split(",").map(function (a) {
                return a.split("::");
              })
      ...
      getFixedDslColumn: function () {
        ...
        col["Check"] = {
          ...
          resizeable: false,
          orderable: false,
          renderable: columnToShow
            ? _.contains(columnToShow, "selected")
            : true,
          ...
          formatter: _.extend({}, Backgrid.CellFormatter.prototype, {
            fromRaw: function (rawValue, model) {
              ...
              if (!obj.attributes || obj.attributes.serviceType === undefined) {
                if (
                  Globals.serviceTypeMap[obj.typeName] === undefined &&
                  ...
                  if (defObj) {
                    Globals.serviceTypeMap[obj.typeName] =
                      defObj.get("serviceType");
                  }
                }
              } else if (Globals.serviceTypeMap[obj.typeName] === undefined) {
                Globals.serviceTypeMap[obj.typeName] = obj.attributes
                  ? obj.attributes.serviceType
                  : null;
              }
              ...
              if (obj.guid) {
                if (obj.guid == "-1") {
                  ...
                  nameHtml =
                    ...
                    '" href="#!/detailPage/' +
                    obj.guid +
                    (that.fromView ? "?from=" + that.fromView : "") +
                    '">' +
                    name +
                    "</a>";
                }
        ...
        if (this.value && !this.profileDBView) {
          ...
          col["typeName"] = {
            ...
            orderable: true,
            renderable: columnToShow
              ? _.contains(columnToShow, "typeName")
              : true,
            formatter: _.extend({}, Backgrid.CellFormatter.prototype, {
              fromRaw: function (rawValue, model) {
                var obj = model.toJSON();
            ...
            if (this.value.type == "_ALL_ENTITY_TYPES") {
              this.businessMetadataDefCollection.each(function (model) {
                ...
                if (sortedAttributes) {
                  ...
                  businessAttributes[name] = $.extend(
                    true,
                    {},
                    sortedAttributes
                  );
              ...
              businessAttributes = def
                ? $.extend(true, {}, def.get("businessAttributeDefs")) || null
                : null;
            }
            if (
              def ||
              Globals[this.value.type] ||
              (this.value.tag
              ...
              if (
                this.value.type &&
                (Globals[this.value.type] || Globals[Enums.addOnEntities[0]])
              ) {
                systemAttr = (
                  Globals[this.value.type] || Globals[Enums.addOnEntities[0]]
                ).attributeDefs;
                ...
                _.each(
                  ...
                  function (businessMetadata, businessMetadataName) {
                    _.each(businessMetadata, function (attr, index) {
                      ...
                      attribute.name =
                        businessMetadataName + "." + attribute.name;
              ...
              _.each(attrObj, function (obj, key) {
                ...
                if (
                  key == "__historicalGuids" ||
                  key == "__classificationsText" ||
                  key == "__classificationNames" ||
                  key == "__propagatedClassificationNames"
                ) {
                  return;
                }
                col[obj.name] = {
                  label: Enums.systemAttributes[obj.name]
                    ? Enums.systemAttributes[obj.name]
                    : _.escape(
                        obj.isBusinessAttributes
                          ? obj.name
                          : obj.name.capitalize()
                      ),
                  cell: "Html",
                  ...
                  headerClassName: obj.isBusinessAttributes
                    ? "no-capitalize"
                    : "",
                  formatter: _.extend({}, Backgrid.CellFormatter.prototype, {
                    fromRaw: function (rawValue, model) {
                      ...
                      if (
                        ...
                        if (key == "__labels") {
                          var values = modelObj.attributes[key]
                              ? modelObj.attributes[key].split("|")
                              : null,
                            valueOfArray = [];
                          if (values) {
                            ...
                            _.each(values, function (names) {
                              valueOfArray.push(
                                '<span class="json-string"><a class="btn btn-action btn-sm btn-blue btn-icon" ><span>' +
                                  _.escape(names) +
                                  "</span></a></span>"
                              );
                            });
                        ...
                        if (key == "__customAttributes") {
                          var customAttributes = modelObj.attributes[key]
                              ? JSON.parse(modelObj.attributes[key])
                              : null,
                            valueOfArray = [];
                          if (customAttributes) {
                            _.each(
                              ...
                              function (value, index) {
                                valueOfArray.push(
                                  '<span class="json-string"><a class="btn btn-action btn-sm btn-blue btn-icon" ><span><span>' +
                                    _.escape(value) +
                                    "</span> : <span>" +
                                    _.escape(
                                      Object.values(customAttributes)[index]
                                    ) +
                                    "</span></span></a></span>"
                                );
                              }
                        ...
                        if (_.isArray(modelObj.attributes[key])) {
                          ...
                          if (tempObj.valueObject[key].length > 2) {
                            column
                              .addClass("toggleList semi-collapsed")
                              .append(
              ...
              formatter: _.extend({}, Backgrid.CellFormatter.prototype, {
                fromRaw: function (rawValue, model) {
                  ...
                  if (key == "name") {
                    ...
                    if (
                      modelObj.status &&
                      Enums.entityStateReadOnly[modelObj.status]
                    ) {
                      nameHtml +=
                        '<button type="button" title="Deleted" class="btn btn-action btn-md deleteBtn"><i class="fa fa-trash"></i></button>';
                      return (
      ...
      getTagList: function (guid, multiple) {
        ...
        if (!multiple || multiple.length === 0) {
          ...
          return _.compact(
            _.map(obj.classifications, function (val) {
      ...
      hideLoader: function (options) {
        ...
        options && options.type === "error"
          ...
          : this.$(".ellipsis-with-margin,.pagination-box").show(); // only show for first time and hide when type is error
      ...
      assignTermModalView: function (glossaryCollection, obj) {
        ...
        if (terms) {
          require(["views/glossary/AssignTermLayoutView"], function (
      ...
      onClickAddTermBtn: function (e) {
        ...
        if (
          ...
          obj.associatedTerms = this.searchCollection
      ...
      onClickTagCross: function (e) {
        ...
        CommonViewFunction.deleteTag({
          ...
          msg:
            "<div class='ellipsis-with-margin'>Remove: " +
            "<b>" +
            _.escape(tagName) +
            "</b> assignment from <b>" +
            _.escape(assetName) +
      ...
      onClickCreateEntity: function (e) {
        ...
        require(["views/entity/CreateEntityLayoutView"], function (
          ...
          if (this.finalPage && this.finalPage < goToPage) {
            Utils.notifyInfo({
              ...
              content:
                Messages.search.noRecordForPage +
                "<b>" +
                Utils.getNumberSuffix({ number: goToPage, sup: true }) +
                "</b> page",
            });
            return;
          }
